...
typedef struct Node {
	...
	Node* addNode(Node* head, FASTArecord* newData) {
...
void processList(Node* head) {
	...
	int i = 0;
    while (current != NULL) {
        //printf("Header: %sSequence: %s\n", current->data->description, current->data->sequence);
        current = current->next;
		i ++;
    }
	printf("Stored %d files into linked list\n\n", i);
}

int fasta_head(char* filename, Node** head) {
    // Node* head = NULL;
    ...
    //         record->description = strdup(line + 1);
    //         if (getline(&line, &len, fp) != -1) {
    //             record->sequence = strdup(line);
    //         }
    //         head = addNode(head, record);
    //         recordCount++;
    //     }
    // }
     while (getline(&line, &len, fp) != -1) {
        if (line[0] == '>') {
            ...
            *head = addNode(*head, record);  // Add the record to the linked list
            recordCount++;
        }
    }
    free(line);
